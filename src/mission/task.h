...
public:
    ...
    #define REG_STATE(STATE, METHOD_NAME, TIMEOUT, FALLBACK_STATE) \
        register_state(STATE, #STATE, &std::remove_pointer<decltype(this)>::type::METHOD_NAME, this, TIMEOUT, FALLBACK_STATE)


    Task(const YamlReader& cfg, ipc::Communicator& comm, State initial_state): TaskBase(cfg, comm)
    {
        state_machine_.switch_state_to(initial_state);
    }
