...
void nmeaMathInfoToPosition(const NmeaInfo *info, NmeaPosition *pos) {
  ...
  pos->lat = nmeaMathNdegToDegree(NMEALIB_LATITUDE_DEFAULT_NDEG);
  pos->lon = nmeaMathNdegToDegree(NMEALIB_LONGITUDE_DEFAULT_NDEG);
  ...
  if (nmeaInfoIsPresentAll(info->present, NMEALIB_PRESENT_LAT)) {
    pos->lat = nmeaMathNdegToDegree(info->latitude);
  }

  if (nmeaInfoIsPresentAll(info->present, NMEALIB_PRESENT_LON)) {
    pos->lon = nmeaMathNdegToDegree(info->longitude);
  }
}
